...
@section('container')
  ...
  <a href="/surat/odr/create" class="btn btn-primary">Buat Surat ODR</a>
      ...
      <tbody>
          @foreach ($odr as $data)
              <tr>
                  ...
                  <td>{{ $data['alamat'] }}</td>
                  <td>
                    <a href="/surat/odr/{{ $data->id }}" class="badge bg-info" target="_blank"><i class="bi bi-printer"></i></a>
                    <a href="/surat/odr/{{ $data->id }}/edit" class="badge bg-warning"><i class="bi bi-pencil"></i></a>
                    <form action="/surat/odr/{{ $data->id }}" method="POST" class="d-inline">
                      @method('DELETE')
                      @csrf
